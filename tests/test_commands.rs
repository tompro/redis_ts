...
fn test_create_ts_no_options() {
    ...
    let r:Value = get_con().ts_create("test_ts_no_op", TsOptions::default()).unwrap();
...
fn test_ts_add() {
    ...
    let _:Value = get_con().ts_create("test_ts_add", default_settings()).unwrap();
...
fn test_ts_add_now() {
    ...
    let _:Value = get_con().ts_create("test_ts_add_now", default_settings()).unwrap();
...
fn test_ts_add_create() {
    ...
    let ts:u64 = get_con().ts_add_create(
        "test_ts_add_create", 1234567890, 2.2, default_settings()
    ).unwrap();
    assert_eq!(ts, 1234567890);
    let ts2:u64 = get_con().ts_add_create(
        "test_ts_add_create", "*", 2.3, default_settings()
    ).unwrap();
    assert!(ts2 > ts);
}
...
fn test_ts_madd() {
    ...
    let _:Value = get_con().ts_create("test_ts_madd", default_settings()).unwrap();
    let _:Value = get_con().ts_create("test_ts2_madd", default_settings()).unwrap();

    let expected:Vec<u64> = vec![1234, 4321];
    let res:Vec<u64> = get_con().ts_madd(&[("test_ts_madd", 1234, 1.0), ("test_ts2_madd", 4321, 2.0)]).unwrap();
...
fn test_ts_incrby_now() {
    ...
    let _:Value = get_con().ts_create("test_ts_incrby_now", default_settings()).unwrap();
...
fn test_ts_decrby_now() {
    ...
    let _:Value = get_con().ts_create("test_ts_decrby_now", default_settings()).unwrap();
...
fn test_ts_incrby() {
    ...
    let _:Value = get_con().ts_create("test_ts_incrby", default_settings()).unwrap();
...
fn test_ts_decrby() {
    ...
    let _:Value = get_con().ts_create("test_ts_decrby", default_settings()).unwrap();
...
fn test_ts_incrby_create() {
    ...
    let _:() = get_con().ts_incrby_create("test_ts_incrby_create", 123, 1, default_settings()).unwrap();
    let v1:Option<(u64,f64)> = get_con().ts_get("test_ts_incrby_create").unwrap();
    assert_eq!(v1.unwrap(), (123, 1.0));

    let _:() = get_con().ts_incrby_create("test_ts_incrby_create", 1234, 5, default_settings()).unwrap();
...
fn test_ts_decrby_create() {
    ...
    let _:() = get_con().ts_decrby_create("test_ts_decrby_create", 123, 1, default_settings()).unwrap();
    let v1:Option<(u64,f64)> = get_con().ts_get("test_ts_decrby_create").unwrap();
    assert_eq!(v1.unwrap(), (123, -1.0));

    let _:() = get_con().ts_decrby_create("test_ts_decrby_create", 1234, 5, default_settings()).unwrap();
...
fn test_ts_create_delete_rule() {
    ...
    let _:Value = get_con().ts_create("test_ts_create_delete_rule", default_settings()).unwrap();
    let _:Value = get_con().ts_create("test_ts_create_delete_rule2", default_settings()).unwrap();
    let _:() = get_con().ts_createrule(
        "test_ts_create_delete_rule", "test_ts_create_delete_rule2", TsAggregationType::Avg(5000)
    ).unwrap();

    let info:TsInfo = get_con().ts_info("test_ts_create_delete_rule").unwrap();
    assert_eq!(info.rules, vec![("test_ts_create_delete_rule2".to_string(), 5000, "AVG".to_string())]);

    let _:() = get_con().ts_deleterule("test_ts_create_delete_rule", "test_ts_create_delete_rule2").unwrap();
    let info:TsInfo = get_con().ts_info("test_ts_create_delete_rule").unwrap();
...
fn test_ts_get() {
    ...
    let _:Value = get_con().ts_create("test_ts_get", default_settings()).unwrap();
...
fn test_ts_get_ts_info() {
    ...
    let _:() = get_con().ts_add("test_ts_get_ts_info", "1234", 2.0).unwrap();
...
fn test_ts_mrange() {
    ...
    let _:() = get_con().ts_create("test_ts_mrange2", opts.clone()).unwrap();
    let _:() = get_con().ts_madd(&[
