...
use redis_ts::{TsInfo, TsOptions, TsAggregationType, TsMget, TsFilterOptions, TsRange, TsMrange};
use async_std::task;
...
fn test_ts_add_create() {
	let _:() = task::block_on(async {
		...
		let ts:u64 = con.ts_add_create(
        "async_test_ts_add_create", 1234567890, 2.2, TsOptions::default()
    ).await.unwrap();
		assert_eq!(ts, 1234567890);

		let ts2:u64 = con.ts_add_create(
        "async_test_ts_add_create", "*", 2.3, TsOptions::default()
    ).await.unwrap();
    assert!(ts2 > ts);
	});
}
...
fn test_ts_madd() {
	let _:() = task::block_on(async {
		...
		let res:Vec<u64> = con.ts_madd(
			&[("async_test_ts_madd", 1234, 1.0), ("async_test_ts_madd2", 4321, 2.0)]
		).await.unwrap();
...
fn test_ts_incrby_now() {
	let _:() = task::block_on(async {
		...
		let _:() = con.ts_incrby_now("async_test_ts_incrby_now", 1).await.unwrap();
    let v1:Option<(u64,f64)> = con.ts_get("async_test_ts_incrby_now").await.unwrap();
    assert_eq!(v1.unwrap().1, 1.0);
    sleep(1);
    let _:() = con.ts_incrby_now("async_test_ts_incrby_now", 5).await.unwrap();
...
fn test_ts_decrby_now() {
	let _:() = task::block_on(async {
		...
		let _:() = con.ts_add_now("async_test_ts_decrby_now", 10).await.unwrap();
		let v1:Option<(u64,f64)> = con.ts_get("async_test_ts_decrby_now").await.unwrap();
    assert_eq!(v1.unwrap().1, 10.0);
    sleep(1);

    let _:() = con.ts_decrby_now("async_test_ts_decrby_now", 1).await.unwrap();
    let v1:Option<(u64,f64)> = con.ts_get("async_test_ts_decrby_now").await.unwrap();
    assert_eq!(v1.unwrap().1, 9.0);
    sleep(1);

    let _:() = con.ts_decrby_now("async_test_ts_decrby_now", 5).await.unwrap();
...
fn test_ts_incrby() {
    ...
    let _:() = con.ts_incrby("async_test_ts_incrby", 1234, 5).await.unwrap();
...
fn test_ts_decrby() {
    ...
    let _:() = con.ts_decrby("async_test_ts_decrby", 1234, 5).await.unwrap();
...
fn test_ts_incrby_create() {
    ...
    let _:() = con.ts_incrby_create("async_test_ts_incrby_create", 123, 1, TsOptions::default()).await.unwrap();
    let v1:Option<(u64,f64)> = con.ts_get("async_test_ts_incrby_create").await.unwrap();
    assert_eq!(v1.unwrap(), (123, 1.0));

    let _:() = con.ts_incrby_create("async_test_ts_incrby_create", 1234, 5, TsOptions::default()).await.unwrap();
...
fn test_ts_decrby_create() {
    ...
    let _:() = con.ts_decrby_create("async_test_ts_decrby_create", 123, 1, TsOptions::default()).await.unwrap();
    let v1:Option<(u64,f64)> = con.ts_get("async_test_ts_decrby_create").await.unwrap();
    assert_eq!(v1.unwrap(), (123, -1.0));

    let _:() = con.ts_decrby_create("async_test_ts_decrby_create", 1234, 5, TsOptions::default()).await.unwrap();
...
fn test_ts_create_delete_rule() {
    ...
    let _:() = con.ts_createrule(
        "async_test_ts_create_delete_rule", "async_test_ts_create_delete_rule2", TsAggregationType::Avg(5000)
    ).await.unwrap();

    let info:TsInfo = con.ts_info("async_test_ts_create_delete_rule").await.unwrap();
    assert_eq!(info.rules, vec![("async_test_ts_create_delete_rule2".to_string(), 5000, "AVG".to_string())]);

    let _:() = con.ts_deleterule("async_test_ts_create_delete_rule", "async_test_ts_create_delete_rule2").await.unwrap();
    let info:TsInfo = con.ts_info("async_test_ts_create_delete_rule").await.unwrap();
...
fn test_ts_mget() {
    ...
    let _:() = con.ts_create("async_test_ts_mget", opts.clone()).await.unwrap();
    let _:() = con.ts_create("async_test_ts_mget2", opts.clone()).await.unwrap();
    let _:() = con.ts_create("async_test_ts_mget3", opts.clone()).await.unwrap();
    ...
    let res:TsMget<u64,f64> = con.ts_mget(
        TsFilterOptions::default().equals("l", "async_mget").with_labels(true)
    ).await.unwrap();
...
fn test_ts_get_ts_info() {
    ...
    let _:() = con.ts_create("async_test_ts_get_ts_info", TsOptions::default().label("a", "b")).await.unwrap();
    let _:() = con.ts_add("async_test_ts_get_ts_info", "1234", 2.0).await.unwrap();
...
fn test_ts_range() {
    ...
    let res:TsRange<u64,f64> = con.ts_range("async_test_ts_range", "-", "+", None::<usize>, None).await.unwrap();
    assert_eq!(res.values, vec![(12, 1.0), (123, 2.0), (1234, 3.0)]);

    let one_res:TsRange<u64,f64> = con.ts_range("async_test_ts_range", "-", "+", Some(1), None).await.unwrap();
    assert_eq!(one_res.values, vec![(12, 1.0)]);

    let range_res:TsRange<u64,f64> = con.ts_range("async_test_ts_range", 12, 123, None::<usize>, None).await.unwrap();
    assert_eq!(range_res.values, vec![(12, 1.0), (123, 2.0)]);

    let sum:TsRange<u64,f64> = con.ts_range("async_test_ts_range", 12, 123, None::<usize>, Some(TsAggregationType::Sum(10000))).await.unwrap();
    assert_eq!(sum.values, vec![(0, 3.0)]);

    let res:TsRange<u64,f64> = con.ts_range("async_test_ts_range2", "-", "+", None::<usize>, None).await.unwrap();
...
fn test_ts_mrange() {
    ...
    let _:() = con.ts_create("async_test_ts_mrange", opts.clone()).await.unwrap();
    let _:() = con.ts_create("async_test_ts_mrange2", opts.clone()).await.unwrap();
    let _:() = con.ts_madd(&[
    ...
    let res:TsMrange<u64,f64> = con.ts_mrange(
        "-", "+", None::<usize>, None, TsFilterOptions::default().equals("l", "async_mrange").with_labels(true)
    ).await.unwrap();
    ...
    assert_eq!(res.values[0].labels, vec![("l".to_string(), "async_mrange".to_string())]);

    let res2:TsMrange<u64,f64> = con.ts_mrange(
...
fn test_ts_queryindex() {
    ...
    let _:() = con.ts_create("async_test_ts_queryindex", TsOptions::default().label("a", "b")).await.unwrap();
    let _:() = con.ts_add("async_test_ts_queryindex", "1234", 2.0).await.unwrap();
    let index:Vec<String> = con.ts_queryindex(TsFilterOptions::default().equals("a", "b")).await.unwrap();
